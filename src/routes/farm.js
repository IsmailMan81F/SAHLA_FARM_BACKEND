...
export function createStatesRouter() {
  ...
  router.get("/states", async (req, res) => {
    ...
    try {
      ...
      if (!state) {
        return res
          .status(503)
          .json({ error: "Home Assistant instance is not connected yet." });
      }
  ...
  router.get("/graph", async (req, res) => {
    try {
      ...
      // ── 2. Verify the token and get the farm_id ────────────────────────────────
      const { unauthorized } = await verifyUser(token);
      ...
      // ── 3. Authenticate and get farm_id ────────────────────────────────────────
      const { authorized, farm_id } = await authenticateClient(token);

      if (!authorized || !farm_id) {
        return res
          .status(403)
          .json({ error: "Not authorized to access this farm" });
      }

      // ── 4. Calculate date range for current week (Sunday to Saturday) ──────────
      const now = new Date();
      ...
      // ── 5. Fetch sensor data from database ─────────────────────────────────────
      ...
      if (sensorError) {
        console.error("[Graph] Error fetching sensor data:", sensorError);
      ...
      // ── 7. Map sensor types to units and group by type ────────────────────────
      const sensorTypeMap = {
        temperature: { type: "temperature", unit: "°C" },
        humidity: { type: "air_humidity", unit: "%" },
        air_humidity: { type: "air_humidity", unit: "%" },
        "soil moisture": { type: "soil_moisture", unit: "%" },
        soil_moisture: { type: "soil_moisture", unit: "%" },
        light_intensity: { type: "light_intensity", unit: "Lux" },
        luminosity: { type: "light_intensity", unit: "Lux" },
      };

      // Group sensor data by normalized type
      const groupedData = {};

      for (const sensor of sensorData) {
        const dbType = sensor.type?.toLowerCase() || "";
        const typeInfo = sensorTypeMap[dbType];
        ...
        if (!groupedData[normalizedType]) {
          groupedData[normalizedType] = {
            sensor: {
              type: typeInfo.type,
              unit: typeInfo.unit,
            },
        ...
        groupedData[normalizedType].data.push({
          id: sensor.id,
      ...
      console.error("[Graph] Unexpected error:", error);
