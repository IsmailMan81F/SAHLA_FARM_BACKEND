...
export function createNotificationsRouter() {
    ...
    try {
      ...
      if (!haResult.offlineMode) {
        return res
          ...
          .json({
            error:
              haResult.message || "Home Assistant credentials are not valid",
          });
      }
      ...
      if (!farmId) {
        return res
          .status(400)
          .json({ error: "Could not determine farm for current user" });
      }
      ...
      const {
        ...
        error: userError,
      } = await fetchUserUnreadNotifications(user_id, userLimit);
      if (!userSuccess) {
      ...
      if (farmLimit > 0) {
        const {
          ...
          notifications,
          error: farmError,
        } = await fetchFarmNotifications(farmId, farmLimit);
        if (!farmSuccess) {
          console.error("Failed to fetch farm notifications:", farmError);
          return res
            .status(500)
            .json({ error: "Failed to load farm notifications" });
        }
  ...
  router.put("/all", async (req, res) => {
    ...
    if (!status) {
      return res
        .status(400)
        .json({ error: "Status query parameter is required (read or unread)" });
    }
    ...
    try {
      ...
      const { success, error, data } = await updateAllNotificationStatus(
        user_id,
        status,
      );
      if (!success) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to update notification statuses" });
    }
  });
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      const { success, error, data } = await updateNotificationStatus(
        user_id,
        id,
        status,
      );
